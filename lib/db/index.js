...
var conString = 'postgres://dljjgyrnmbxpbr:nKqGR9lybFjQxIt5XJk8feIJ7F@ec2-54-83-204-104.compute-1.amazonaws.com:5432/d3j3n73u9o6ffs';
        ...
        if(err){
            ...
            client.query(str, [uid], function(err, result) {
                ...
                if(err)
                    ...
                    cb(undefined, result.rows[0]); // or is it result.rows[0].object ?
            });
        }
    });
}
...
exports.uidList = function() {
    var arr = [];

    var len = objectdb.length;
    for(var i = 0; i < len; i++) {
        arr.push({ "url" : "api/objects/" + objectdb[i].uid })
    }
